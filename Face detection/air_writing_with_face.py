"""
╔══════════════════════════════════════════════════════════════╗
║            ◈  AIR CANVAS PRO  ◈  v3.0 ULTRA                ║
║   Beyond Human Imagination — AI-Powered Gesture Drawing      ║
╚══════════════════════════════════════════════════════════════╝

GESTURE CONTROLS:
  ☝  Index Only       → DRAW (velocity-adaptive brush)
  ✌  Peace Sign       → ERASE (radius eraser)
...
KEYBOARD:
  ...
  S  → Save manually
  M  → Cycle mode
  Q  → Quit (auto-saves final)

DRAWING MODES:
  FREEHAND    — Classic smooth drawing (Kalman-filtered)
  SYMMETRY    — Mirror drawing (horizontal + vertical)
  RADIAL      — 4-way radial symmetry
  CONSTELLATION — Connect nearby points with star lines
  GLOW        — Neon bloom effect
  RAINBOW     — Time-cycling chromatic strokes
  SPEED_ART   — Brush size driven by movement velocity
"""
...
AUTO_SAVE_SEC     = 30
MAX_PARTICLES     = 150
CONSTELLATION_R   = 160   # max distance to connect constellation points
MAX_CONST_PTS     = 60
ERASE_RADIUS      = 28
FIST_HOLD_SEC     = 1.0
VELOCITY_SMOOTH   = 0.25  # EMA alpha for velocity
...
# ══════════════════════════════════════════════
#  KALMAN FILTER  (smooth finger trajectory)
# ══════════════════════════════════════════════
class Kalman1D:
    """Minimal scalar Kalman filter for 1-D tracking."""
...
def rainbow_color(t: float):
    """HSV-cycling rainbow. t = time in seconds."""
    hue = int((t * 60) % 180)
...
GESTURE_MAP = {
    ...
    (0,0,0,0,0): "FIST",
    (1,1,1,1,1): "OPEN_PALM",
    (0,1,1,1,1): "OPEN_PALM",   # thumb tucked
    (1,0,0,0,0): "THUMBS_UP",
    (0,1,0,0,1): "ROCK",
    (0,1,1,1,0): "THREE",
}
...
# ══════════════════════════════════════════════
#  EMOTION DETECTION  (from face mesh)
# ══════════════════════════════════════════════
def detect_emotion(lms):
    try:
        ...
        avg_corner_y = (lms[61].y + lms[291].y) / 2
...
# ══════════════════════════════════════════════
#  FACE WINDOW WITH MESH OVERLAY
# ══════════════════════════════════════════════
def draw_face_window(frame, face_result, fw=150, fh=150):
    """Draws face crop + landmark dots in top-left corner."""
    h, w = frame.shape[:2]
    ...
    for i, p1 in enumerate(pts):
        ...
        for p2 in pts[i+1:]:
            ...
            if d < CONSTELLATION_R:
                ...
                line_c = tuple(min(255, int(c * intensity / 180)) for c in color)
                cv2.line(canvas, p1, p2, line_c, 1)
...
def radial_draw(canvas, px, py, ppx, ppy, cx, cy, color, thickness):
    """Draw 4-way rotationally-symmetric strokes."""
    pts = [
        ((px, py), (ppx, ppy)),
        ((2*cx - px, py), (2*cx - ppx, ppy)),       # horiz flip
        ((px, 2*cy - py), (ppx, 2*cy - ppy)),        # vert flip
        ((2*cx - px, 2*cy - py), (2*cx - ppx, 2*cy - ppy)),  # both
    ]
    for (a, b) in pts:
...
#  HOLOGRAPHIC HUD
# ══════════════════════════════════════════════
def draw_hud(frame, gesture, fps, emotion, em_color, brush, mode_name,
             color_name, draw_color, undo_cnt, paused, vel):
    h, w = frame.shape[:2]
    PAD = 8
    ...
    panel_x = w - 195
    ovl = frame.copy()
    cv2.rectangle(ovl, (panel_x - PAD, 0), (w, 320), (5, 5, 15), -1)
    cv2.addWeighted(ovl, 0.45, frame, 0.55, 0, frame)
    cv2.rectangle(frame, (panel_x - PAD, 0), (w, 320), (0, 200, 200), 1)
    ...
    put(f"GESTURE : {gesture}", 52, (0,255,255), 0.48, 1)
    put(f"MODE    : {mode_name}", 76, (255,200,0), 0.48, 1)
    put(f"VELOCITY: {vel:4.0f}", 100, (180,180,180), 0.45)

    # Color swatch
    put("COLOR", 128, (180,180,180), 0.45)
    cv2.rectangle(frame, (panel_x, 135), (panel_x+55, 155), draw_color, -1)
    cv2.rectangle(frame, (panel_x, 135), (panel_x+55, 155), (255,255,255), 1)
    put(color_name, 170, (220,220,220), 0.42)

    # Brush size bar
    put(f"BRUSH {brush}px", 195, (180,180,180), 0.45)
    bar_w = int(185 * brush / BRUSH_MAX)
    cv2.rectangle(frame, (panel_x, 200), (panel_x+185, 208), (40,40,40), -1)
    cv2.rectangle(frame, (panel_x, 200), (panel_x+bar_w, 208), draw_color, -1)

    put(f"MOOD : {emotion}", 232, em_color, 0.42)
    put(f"UNDO STACK : {undo_cnt}/{MAX_UNDO}", 258, (150,150,150), 0.42)

    if paused:
        put("⏸  PAUSED", 290, (0,200,255), 0.55, 2)

    # ── Bottom bar ───────────────────────────────
    bar_h = 58
    ovl2 = frame.copy()
    cv2.rectangle(ovl2, (0, h-bar_h), (panel_x-PAD, h), (5,5,15), -1)
    cv2.addWeighted(ovl2, 0.5, frame, 0.5, 0, frame)
    cv2.line(frame, (0, h-bar_h), (panel_x-PAD, h-bar_h), (0,200,200), 1)

    shortcuts = [
        ("☝ INDEX","Draw"), ("✌ PEACE","Erase"), ("✊ FIST 1s","Clear"),
        ("🖐 PALM","Pause"), ("👍 THUMB","Save"),
        ("🤟 ROCK","Mode"), ("3 FIN","Color"), ("Z","Undo"), ("Q","Quit"),
    ]
    n   = len(shortcuts)
    slot = (panel_x - PAD) // n
...
def draw_skeleton(frame, lms, w, h):
    for conn in mp_hands.HAND_CONNECTIONS:
        ...
        cv2.line(frame, (ax,ay), (bx,by), (40,40,60), 1)
    for lm in lms:
...
vel_smooth         = 0.0       # smoothed velocity px/frame
brush_size         = 8
frame_idx          = 0
fps                = 30.0
fps_t0             = time.time()
last_auto_save     = time.time()
cx, cy             = 0, 0      # canvas center (for radial mode)

constellation_pts: list[tuple] = []

print("╔══════════════════════════════════════╗")
print("║   ◈  AIR CANVAS PRO  v3.0 ULTRA    ║")
print("╚══════════════════════════════════════╝")
print(f"📁 Drawings → {os.path.abspath(SAVE_DIR)}")
print("☝ Index = Draw  |  ✌ Peace = Erase  |  ✊ Fist 1s = Clear")
print("🖐 Palm = Pause  |  👍 Thumb = Save  |  🤟 Rock = Mode  |  3 Fin = Color")
...
while True:
    ...
    if hand_result.multi_hand_landmarks:
        ...
        if gesture != prev_gesture:
            fist_t = None          # reset fist timer on gesture change
        ...
        if not paused or gesture == "OPEN_PALM":
            ...
            # ——— DRAW ———
            if gesture == "DRAW":
                # Depth simulation: wrist-to-tip distance → brush size
                wx, wy = lms_list[0].x, lms_list[0].y
                ix, iy = lms_list[8].x, lms_list[8].y
            ...
            # ——— OPEN PALM (toggle pause) ———
                ...
                paused = not paused if prev_gesture != "OPEN_PALM" else paused
                prev_x, prev_y = None, None
    ...
    # ── HUD ──────────────────────────────────
    c_name, c_val = PALETTE[color_idx]
    draw_hud(
        output,
        gesture   = gesture,
        fps       = fps,
    ...
    if paused:
        ...
        cv2.putText(output, "⏸  PAUSED  —  Show Open Palm to Resume",
                    (w//2 - 260, h//2),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0,255,255), 2, cv2.LINE_AA)

    # ── SAVE FLASH ───────────────────────────
    if saved_flash > 0:
        cv2.putText(output, "💾  SAVED!", (w//2 - 70, 80),
...
print("\n✅ Air Canvas Pro closed.")
print(f"📁 All drawings saved in: {os.path.abspath(SAVE_DIR)}")
